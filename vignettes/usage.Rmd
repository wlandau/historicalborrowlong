...
As explained in the `hbl_data()` and `hb_mcmc_*()` help files, before running the MCMC, dataset is pre-processed.
...
mcmc <- hbl_mcmc_sge(
  ...
  rep_reference = "visit1",
   # Can be continuous, categorical, or binary columns:
  covariates = c("factor1", "factor2"),
  s_tau = 0.5, # The user should to choose this carefully.
...
```{r, warning = FALSE, output = FALSE, message = FALSE, eval = TRUE, echo = FALSE}
suppressWarnings(
  mcmc <- hbl_mcmc_sge(
    data = data,
    response = "outcome",
...
The hierarchical model is the primary model of interest. For a small number of historical studies, it may be necessary to choose a value of `s_tau` to approximate the prior amount of borrowing given a desired precision ratio and supposed residual variance. See the function `hbl_s_tau()` for how to do this. For a large number of historical studies, the computation may take an extremely long time to run. It may be necessary to change arguments `covariance_current` and `covariance_historical` (especially the latter) for computational efficiency, but not even that may be enough if the data is enormous.
...
```{r, warning = FALSE, output = FALSE, echo = FALSE, eval = TRUE}
suppressWarnings(
  mcmc_hierarchical <- hbl_mcmc_hierarchical(
    data = data,
    response = "outcome",
...
```{r, warning = FALSE, output = FALSE, eval = TRUE, echo = FALSE}
suppressWarnings(
  mcmc_pool <- hbl_mcmc_pool(
    data = data,
    response = "outcome",
