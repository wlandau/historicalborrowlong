test_that("sim independent data", {
  set.seed(0)
  out <- hbl_sim_independent(
    n_study = 2,
    n_group = 3,
    n_patient = 5,
    n_rep = 2,
...
test_that("sim independent parameters with constraint", {
  set.seed(0)
  out <- hbl_sim_independent(
    n_study = 2,
    n_group = 3,
  ...
  expect_equal(length(out$alpha), 6)
...
test_that("sim independent parameters without constraint", {
  set.seed(0)
  out <- hbl_sim_independent(
    n_study = 2,
    n_group = 3,
...
test_that("sim independent unstructured + ar1", {
  set.seed(0)
  out <- hbl_sim_independent(
    n_study = 2,
    n_group = 3,
...
test_that("sim independent ar1 + diagonal", {
  set.seed(0)
  out <- hbl_sim_independent(
    n_study = 2,
    n_group = 3,
...
test_that("sim independent matrices", {
  set.seed(0)
  out <- hbl_sim_independent(
    n_study = 2,
    n_group = 3,
  ...
  x_alpha <- out$x_alpha
  exp <- do.call(rbind, replicate(5, diag(4), simplify = FALSE))
  study2 <- rbind(exp, matrix(0, nrow = 40, ncol = 4))
  exp <- as.matrix(Matrix::bdiag(list(exp, study2)))
